...
- `backend` 已经把整数最小主链路拆成 `fetch/decode -> decoded uop queue -> rename/ROB alloc -> issue queue wakeup/select -> issue reg -> regread -> execute -> execute result reg`。
- `backend` 在 `O3_SIM` 宏下已支持逐周期文本调试，按 cycle 把 `DECODE/RENAME/WAKEUP/ISSUE/REGREAD/EXECUTE` 各级组织成一个日志块输出。
...
- `issue_queue` 已新增，负责把 rename 完成后的整数 uop 按 lane 顺序压入单一整数 issue 队列，并在队列内完成默认唤醒、按年龄选择和压缩补位。
- `physical_regfile` 已接入 backend 主链路，当前 reset 后清零，用于整数 regread 阶段。
...
## 当前后端数据流
- 当前数据流是：`frontend -> fetch_entry buffer -> decoder -> decoded uop queue -> free_list + rename_map_table + rob -> integer issue queue -> ALU issue reg -> regread -> execute -> execute result reg`
...
- `issue_queue` 当前对旧表项采用“下一拍默认全部 ready”的占位 wakeup 逻辑。
- 被选中的整数 uop 会进入 3 路 ALU 流水寄存器，随后完成 regread、立即数扩展和 execute。
- 当前还没有结果写回 physical regfile，也没有真实写回广播和提交链路。

## 模块说明
### backend
- 职责：承接 frontend 指令组，驱动 decode queue 与基础 rename 流程。
- 当前实现：fetch buffer + 解码 + decoded uop queue + 物理寄存器分配请求 + rename map 更新 + 最小 ROB 分配/存储 + rename 后整数 uop 入 issue queue + issue/select + regread + execute + result reg。
- 调试能力：当定义 `O3_SIM` 时，backend 固定按周期块输出 `DECODE/RENAME/WAKEUP/ISSUE/REGREAD/EXECUTE`；其中 `RENAME` 行可通过 DPI-C 调用 RV64I 反汇编 helper 显示汇编字符串。
...
- 当前表项字段：
  ...
  - `int_alu_op`
- 当前 ready 位策略：
  - 当前还没有物理寄存器 ready table 和真实写回广播网络。
  - rename 入队时仍采用保守初始化：不需要读取的源操作数视为 ready；立即数形式的第二操作数视为 ready；真正依赖寄存器结果的源操作数先记为 not-ready。
  - 进入 queue 以后，下一拍默认把旧表项全部唤醒。
- 当前未做：真实广播唤醒、flush/rollback 恢复、跨功能单元的更复杂选择仲裁。

### physical_regfile
- 职责：提供物理寄存器存储体。
- 当前实现：已接入 backend 的 regread 阶段，支持多读端口和同拍写后读旁路；当前 reset 后所有物理寄存器清零。
- 当前未做：还没有接真实写回网络。
...
- `issue_queue` 对旧表项组合地产生 wakeup 后视图，并按队列顺序把最靠前的 ready 表项分配给编号更小的 ALU issue 端口。
...
- 如果 `rename_fire=1`，则当前 `uop_queue` 队头这组 uop 完成本版 rename；其中所有整数 uop 会按 lane 从小到大连续写入 `issue_queue`。
- `issue_queue` 会把上一拍已经在队列中的未 ready 表项默认唤醒，并删除本拍已经被接受发射的表项。
...
- 当定义 `O3_SIM` 时，本拍会输出一个 lane0 日志块：
  ...
  - `WAKEUP` 行显示本拍被默认唤醒的 queue 内指令 id。
...
- `issue_queue` 对上一拍已经在队列中的未 ready 表项默认置 ready。
