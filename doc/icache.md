...
miss 发生时立即决定并锁存本次 refill 的 victim way，不能等到 response 返回时再选择。

- 优先选择当前 miss set 中的 invalid way。
- 如果所有 way 都 valid，则使用 `rtl/common/lfsr.sv` 中的 LFSR 当前值选择 victim way。
- LFSR 状态在该 miss/refill 流程走到 `DONE` 后更新一次。
