...
constexpr int kExpectedRetires = 5;

// ============================================================================
// Test Focus: backend.sv checkpoint table snapshot timing
// ============================================================================
//
// Circuit behavior under test:
//   When a branch sits in rename lane 3, lanes 0-2 of the same group have
//   already updated the rename_map_table. The checkpoint allocated for that
//   branch must capture the POST-rename mapping (i.e. after lane 0-2
//   updates). The current implementation captures the PRE-rename mapping
//   (start-of-cycle map_table_q), so on recovery the mapping is rolled back
//   to stale preg assignments.
//
// Program layout:
//   0x00: addi x1, x0, 2      ; lane0 -- updates x1->new_preg (value=2)
//   0x04: addi x2, x0, 1      ; lane1
//   0x08: addi x3, x0, 1      ; lane2
//   0x0c: bne  x1, x2, +16    ; lane3 -- branch, checkpoint here
//   wrong-path 0x10-0x18
//   0x1c: add  x0, x1, x0     ; redirect target -- rd=x0, so NO preg is
//                               allocated (p0-release bug cannot affect this)
//
// Expected vs actual:
//   Expected: checkpoint saves x1->p32 (post-rename). After recovery,
//            add x0,x1,x0 reads p32(=2). Retire rd_wdata = 2.
//   Actual:   checkpoint saves x1->p1 (pre-rename). After recovery,
//            x1->p1 which was released at retire, value=0.
//            Retire rd_wdata = 0.
// ============================================================================
...
constexpr uint32_t kAddX0X1X0      = 0x00008033u;  // 0x1c: add  x0, x1, x0
...
constexpr std::array<ExpectedRetire, kExpectedRetires> kExpected = {{
    ...
    {
        // The critical retire: reads x1 whose mapping was restored from
        // checkpoint. rd=x0 means no preg allocation, so p0-release bug
        // cannot mask the checkpoint-snapshot bug.
        .pc = 0x1c,
        .instruction = kAddX0X1X0,
        .uop_type = kRetireUopInt,
        .rd = 0,
        .rd_write_en = false,
        .rd_wdata = 2,  // x1 should be 2 if checkpoint captured post-rename
        .branch_taken = false,
        .branch_mispredict = false,
        .branch_target_pc = 0,
        .branch_fallthrough_pc = 0,
...
void require_field(bool condition, std::string_view message) {
    if (!condition) {
        std::cerr << "[core_three_alu_checkpoint_snapshot][assert] " << message << "\n";
        std::exit(1);
    }
}
...
void check_retire_info(const Vo3_core& dut, int cycle, int& observed_retires) {
    for (int port = 0; port < kRetirePorts; ++port) {
        ...
        std::cout << "[core_three_alu_checkpoint_snapshot][cycle=" << std::dec << cycle
...
int main(int argc, char** argv) {
    ...
    if (observed_retires == kExpectedRetires) {
        require_field(dut.retired_inst_count_o == kExpectedRetires,
                      "retired_inst_count should be 5 at test exit");
        std::cout << "[core_three_alu_checkpoint_snapshot] PASS"
                  << " retired_inst_count=" << std::dec << dut.retired_inst_count_o
                  << " cycles=" << (cycle - 1)
                  << "\n";
    } else {
        std::cout << "[core_three_alu_checkpoint_snapshot] timeout at cycle " << std::dec << cycle
